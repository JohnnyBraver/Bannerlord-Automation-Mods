...
namespace TradeOptimizer
{
    ...
    public static class TradingEngine
    {
            ...
            HashSet<string>? excludedItems = null)
        {
            ...
            // usableCapacity = startWeight + balance (= usableLimit, by definition of CargoCapacityBalance)
                            ...
                            if (isBuyCandidate)
                            {
                                ...
                                else // Balanced
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        bool isCargoFull = (fullness >= limit) || tradeContext.NeedsToSellCargo;
                                        if (!isCargoFull)
                                        {
                                        ...
                                        {
                                            string reason = tradeContext.NeedsToSellCargo ? "NeedsToSellCargo" : $"Usable Cargo Fullness {fullness:P0} >= {limit:P0}";
                                            WriteLog($"[Sell Check Conflict] {itemObj.Name}: Profitable to sell, price below buy threshold. Stance: Balanced ({reason}) -> SELL TO FREE SPACE.");
                                        }
                                    }
                                }
                            }
